...
<!-- Local override of the pi-subagents builtin reviewer (2026-09-20).
Rationale: the builtin's tools field declares watchdog_diff, but no child-loadable
provider exists for it (pi-subagents src/watchdog/diff-tool.js is an internal
factory module, not a Pi extension entry), so every child launch of the builtin
fails strict allowlist validation ("requested unavailable child tools:
watchdog_diff" — reproduced twice). Fix: drop the tool here and require supplied
diff artifacts instead. Revisit on pi-subagents upgrades if watchdog gains a
child-loadable extension entry (then restore the builtin or add
...
- For working-tree delta or committed-range review, require a supplied diff artifact in the task. Without one, report that limitation rather than claiming the working tree or commit was reviewed.
